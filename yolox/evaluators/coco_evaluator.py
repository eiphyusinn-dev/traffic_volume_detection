...
class COCOEvaluator:
    ...
    def convert_to_coco_format(self, outputs, info_imgs, ids, return_outputs=False):
        ...
        for (output, img_h, img_w, img_id) in zip(
            ...
            print(f' bboxes > {bboxes.shape[0]}')
            image_wise_data.update({
                int(img_id): {
                    "bboxes": [box.numpy().tolist() for box in bboxes],
                    "scores": [score.numpy().item() for score in scores],
                    "categories": [
                        self.dataloader.dataset.class_ids[int(cls[ind])] if int(cls[ind]) < len(self.dataloader.dataset.class_ids) else None
            ...
            for ind in range(bboxes.shape[0]):
                ...
                if int(cls[ind]) < len(self.dataloader.dataset.class_ids):
                    ...
                    label = None  # or handle it in another way if needed
